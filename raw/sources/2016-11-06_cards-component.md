...
*Left : Scanning products in a card-based list is challenging to quickly find basic information such as price, since the layout is not predictably arranged, and a user can't easily anticipate where to look on the page to find that information. Right : A vertical list easily supports scanning for price from one product to the next, as this information is placed in a predictable location on each item in the list.*
...
*Salesforce's dashboard system uses cards effectively to show heterogenous content and media together. Clicking on any card produces a full, detailed report for that item. Items have different sizes and are clearly separated from each other. Image attribution*
