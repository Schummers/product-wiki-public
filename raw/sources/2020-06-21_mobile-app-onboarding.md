---
title: "Mobile-App Onboarding: An Analysis of Components and Techniques"
date: "2020-06-21"
...
- [Skip Onboarding Whenever Possible](#toc-skip-onboarding-whenever-possible-1)
- [When Does a Mobile App Need Onboarding?](#toc-when-does-a-mobile-app-need-onboarding-2)
- [What’s Out There: Onboarding Components](#toc-whats-out-there-onboarding-components-3)
- [Feature Promotion](#toc-feature-promotion-4)
- [Customization](#toc-customization-5)
- [Instructions](#toc-instructions-6)
...
- **Memory strain**. Oftentimes onboarding is used to help users remember certain things about the interface (like what an icon means or how to do something), but human memory is limited. Instead of taxing user’s memory by asking them to remember several things about your app, build on existing [mental models](https://www.nngroup.com/articles/mental-models/), and spend time making the app easier to use.
- **May not improve user performance**. We know some of the drawbacks of onboarding, but the benefits are sometimes less clear. However, our research on deck-of-cards tutorials, a method of instructional onboarding, showed that [tutorials didn’t improve task performance](https://www.nngroup.com/articles/mobile-tutorials/).
...
Users always need (often minimal) time to learn a new app, but that doesn’t mean that all apps require separate onboarding flows or lengthy explanations. For most mobile apps, **users should be able to learn the interface by using it and thus**[instructional onboarding flows are not necessary](https://www.nngroup.com/articles/mobile-tutorials/?lm=mobile-instructional-overlay&pt=article). Even for fairly complex mobile apps, it’s often more effective to train users by showing them [tips in context](https://www.nngroup.com/articles/mobile-instructional-overlay/) instead of presenting them with a tutorial that explains the app’s UI — people are simply unlikely to (even try to) [remember](https://www.nngroup.com/articles/working-memory-external-memory/) a lot of information, especially if they are not sure whether they will actually have to use it. Also, while errors are a negative element of user experience, [error messages can teach users](https://www.nngroup.com/articles/errors-forms-design-guidelines/) a bit more about the app in a “teachable moment” where people have a small degree of motivation to read a few words about how to fix the problem.
...
![There are three common components found in onboarding flows: feature promotion, customization, and instructions.](https://media.nngroup.com/media/editor/2020/05/06/onboardingasset-6desktop-2.png)
...
![There are three common components found in onboarding flows: feature promotion, customization, and instructions.](https://media.nngroup.com/media/editor/2020/05/07/onboardingasset-5mobile-2.png)
...
*The Chase banking app prompted existing users with a*[deck-of-cards](https://www.nngroup.com/articles/two-basic-hypertext-presentation-models/)*feature onboarding flow to highlight new app functionalities (like a credit card spending summary). The flow was short and even allowed users to skip it altogether.*
...
Many applications request user data so they can [customize](https://www.nngroup.com/articles/customization-personalization/) that user’s experience. For example, users might customize the content or visual design of an app. However, not all customization should be done during onboarding.

In particular,**visual-design customization, such as selecting a color scheme, doesn’t belong in onboarding.** It’s hard for people to know how they will prefer the app to look before actually using it or why a certain visual design may be better than other. (Also, in general, a lot of research shows that people usually stick with [defaults](https://www.nngroup.com/articles/the-power-of-defaults/). Instead of making them choose and increasing the cognitive load, do your homework, run a study, and figure out which design is best.)
...
[Instructional overlays and coach marks](https://www.nngroup.com/articles/mobile-instructional-overlay/) are another type of instructional onboarding, used to show users where some core functionalities are located within the UI and what those elements do. When implementing instructional overlays, ensure the content is timely (like at a user’s first encounter with a feature) and unobtrusive. This type of onboarding pairs well when it accompanies a user trying to complete a task for the first time, and as such, provides extra nuggets of information as the user progresses. For this reason, **instructional overlays are often nice-to-have and not need-to-have** elements.

*The NOAA Weather app overloaded users by highlighting every possible interaction on one screen. This implementation was highly obtrusive and unnecessary for standard UI elements (like the* Share *and* Settings *icons).**Checklist used instructional overlays to show users some core functionalities, like how to categorize and tag a packing list. However, the instructional copy is used to explain unfamiliar verbiage (left: in this case, a ‘category’ when discussing a packing list) and an unlabeled icon (right: the large orange tag). In this case, a less obtrusive solution for instructional content would be a*[popup tip](https://www.nngroup.com/articles/tooltip-guidelines/)*so that, if a user needed clarity, it would be a tap away. (Even better: improve the UI instead of providing clarity on a bad one).*
