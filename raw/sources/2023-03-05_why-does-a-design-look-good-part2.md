...
author: "Kelley Gordon, Sarah Gibbons"
topics: [visual-design]
type: article
---

In a [previous article](https://www.nngroup.com/articles/why-does-design-look-good/) we broke down why three designs look good. In this article we will analyze four additional user-interface designs and discuss the visual-design principles that make them attractive. Afterall, how something [looks affects the perception of how well it functions.](https://www.nngroup.com/articles/aesthetic-usability-effect/)

## In This Article:

- [Example 1: Comfortable Editing Experience](#toc-example-1-comfortable-editing-experience-1)
- [Example 2: Modular Grid](#toc-example-2-modular-grid-2)
- [Example 3: Monochromatic Color Palette and Symmetrical Imagery](#toc-example-3-monochromatic-color-palette-and-symmetrical-imagery-3)
- [Example 4: Simple and Plain](#toc-example-4-simple-and-plain-4)
- [Conclusion](#toc-conclusion-5)

## Example 1: Comfortable Editing Experience

Our first example is from [Dovetail](https://dovetailapp.com/), a data-analysis platform. This design used a grid, a type system, and consistent iconography to create a comfortable editing and reading experience.

*Dovetail created a visually refined, yet comfortable editing environment.**Dovetail used a basic column grid, an airy text block, various type styles, and visually consistent icons to create a pleasant editing experience.*

**Alignment to a grid.** A 3-column [grid](https://www.nngroup.com/articles/using-grids-in-interface-designs/) acted as a base to which elements are aligned. The leftmost grid line provided a vertical anchor for the sidebar navigation. The middle column of text was also left aligned to a grid line, and the rightmost grid line separated the tags and comments area. Setting up a grid in your designs will keep elements aligned consistently across different screens.

**Type styles for different types of content.** Dovetail used a single font family to provide a consistent typographic experience. Within this font family, there were multiple type text styles and color variations (navy blue and gray). Each variation was used to differentiate between types of content, thus taking advantage of the [Gestalt law of similarity](https://www.nngroup.com/articles/gestalt-similarity/). For example, in the sidebar navigation, *Add a view* and *New tag board* are both unbolded and gray indicating they were the same type of action (creation Avoid using many different type styles in your design, so you do not overwhelm your users.

**Text-line spacing and length.** The large text block was airy due to its generous [leading](https://www.nngroup.com/articles/typography-terms-ux/) (i.e., the distance between the baselines of two consecutive lines of text) and, together with the relatively short line length, allowed for comfortable reading (and editing) experience, since the eye could easily locate the next line down.

**Visually consistent icons.** The many icons used in Dovetail’s app shared line thickness, slightly rounded corners, and looked similar in size. These small details create a unified icon set and bring polish to your design. However, remember that icons are always more easily recognized and, according to [Fitts’s law](https://www.nngroup.com/articles/fitts-law/), faster to tap if [they have labels](https://www.nngroup.com/articles/icon-usability/).

## Example 2: Modular Grid

Our second example is from [MoMA (Museum of Modern Art)’s website](https://www.moma.org/). This design utilizes a modular grid structure, a fun color palette, and imagery to create an exciting, appealing visual experience.

*MoMA’s website is complex and layered, yet pleasing to the eye.**MoMA uses playful colors, meaningful imagery, and a paired back, yet bold content styling.  All design elements are asymmetrically balanced and aligned to a modular grid structure.*
...
**Meaningful imagery.** Images provide a glimpse into what each exhibit offers, rather than being purely decorative. They are cropped appropriately, with the most important element still in view. Your images should add value to your page — for example, by adding supplementary information and detail about a product or concept. Make sure your cropped images make sense, [especially at different screen sizes](https://www.nngroup.com/articles/big-pictures-small-screens/).
...
Our third example is from [Aesop](https://www.aesop.com/), a luxury skincare company. This design utilizes a monochromatic color palette and negative space to create a refined, clean interface.

![Aesop Example](https://media.nngroup.com/media/editor/2023/01/10/aesop-example.png)

*Aesop’s website is aesthetically minimal, refined, and balanced. This page shows a product comparison table, with the item in the middle in focus (cursor hovering on it).*

![Aesop Visual Design Breakdown NNg](https://media.nngroup.com/media/editor/2023/01/10/aesop-example-nng-visual-design-annotations.png)

*Aesop leverages a three-column grid, monochromatic color palette, consistent heights, and symmetrical imagery to achieve a visually pleasing website.*

**Monochromatic color palette.**[Color palettes](https://www.nngroup.com/articles/color-enhance-design/) are sets of colors that have been selected for a particular project, brand, or set of designs. Each individual color plays a role and adds to the visual appeal of the interface. Aesop’s monochromatic color palette is harmonious — though the muted yellows used for the background are similar, slight variations differentiate among the navigation and title area, the product area, and the hover state (in the screenshot above, the item in the middle). Gray and black work together to create a visual hierarchy for typography (black for primary text and gray for secondary).
...
![Aesop Example of Consistent Secondary Info](https://media.nngroup.com/media/editor/2023/01/10/aesop-secondary-example-nng.png)

*Each product’s secondary information (and the lines between them) are left aligned, making it easy to scan and compare. This visual system, that we highlighted in purple, is reused on individual item pages to organize present additional product characteristics.*
...
**Consistent, fixed heights.** When designing [responsive](https://www.nngroup.com/articles/responsive-web-design-definition/) websites, widths, heights, and paddings may adjust depending on the overall width of your user’s window. However, within each responsive range, consistent, fixed widths help designs look aesthetically pleasing and refined. In the above Aesop example, interface elements such as the *Filter by* button, the *Suited to*’ and *Skin feel* secondary information rows, and the *Add to cart* button have the same height, regardless of window width.
...
![Super Human Example](https://media.nngroup.com/media/editor/2023/01/10/super-human-example.png)

*Superhuman’s native app uses grayscale, is primarily textual, and is void of traditional branding. It is good-looking, despite its simplicity.*

![Super Human Visual Example with Annotations NNg](https://media.nngroup.com/media/editor/2023/01/10/super-human-visual-example-nng.png)

*Superhuman uses scale, subtle gradients, and background color to create a clear hierarchy and focus the user’s attention.*
...
- Use only two colors and avoid contrasting colors. Ideally, use only two monochromatic or analogous colors (groups of colors next to each other on the color wheel) that coordinate with your color scheme).
- Introduce an opacity on the drop shadow to help the gradient blend with the rest of the interface.
- Consider the light source and keep it consistent if using multiple drop shadows or gradients (for example, the direction of light to dark should be the same across gradients or the shadow should fall in the same direction across drop shadows)
- Match the direction of your drop shadow to the shape to which you are adding the gradient (for example, linear gradients for squares and radial for circles).
...
![Color Scheme Examples](https://media.nngroup.com/media/editor/2023/01/10/color-scheme-examples-nng.png)

*Avoid choosing foreground and background colors that are too high contrast. Instead, choose monochromatic colors that are subtle but create a clear distinction.*

**Varying type styles within a consistent font family.** Superhuman uses different type styles from within the same [font family](https://www.nngroup.com/articles/typography-terms-ux/) (small caps and bold) and slight changes in color (green, black, and gray) to differentiate between different types of content and focus the user. For example, *Draft* is the same font and size as the rest of the header *to Laura Shea*, but the change in color keeps the design from feeling flat and makes it easy to scan, while also appropriately differentiating the state of the email (draft versus sent).
...
- Dates in small caps (for example, FEB 20)
- Bold for integrations (for example, **LinkedIn** and **Facebook**)
